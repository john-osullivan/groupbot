...
def task_list(group_codename):
    ...
    (infonav, this_group) = gbot.views.get_context(groupcode_name=group_codename)
...
def task_create(group_codename):
    ...
    if current_member is not None:
        ...
        form.delivering_members.choices = gbot.views.get_select_list_for_members(this_group.members)
        form.approving_members.choices = gbot.views.get_select_list_for_members(this_group.members)
        ...
        # However, if they have, we call the controller function, flash messages, and redirect to the detail view.
            ...
            new_task = gbot.controllers.task.create_task(request, this_group.group_id)
            flash("Nice!  You just made a new Task called " + new_task.name + ", good on you.", "success")
            return redirect(url_for(gbot.views.task.task_detail, group_codename=group_codename, task_id=new_task.task_id))
...
def task_detail(group_codename, task_id):
    ...
    (infonav, this_group, this_task) = gbot.views.get_context(group_codename=group_codename, task_id=task_id)
    current_member = gbot.views.get_current_member(group_codename=group_codename, current_user=current_user)
...
def task_edit(group_codename, task_id):
    ...
    if this_task.is_approving(current_member):
        ...
        form.approving_members.default = [list_option['codename'] for list_option in
                                          gbot.views.get_select_list_for_members(this_task.approving_members)]
        form.delivering_members.default = [list_option['codename'] for list_option in
                                          gbot.views.get_select_list_for_members(this_task.delivering_members)]
        ...
        if not form.validate_on_submit():
            ...
            try:
                gbot.controllers.task.edit_task(task_id, request)
                flash("Nice -- you just successfully edited the Task called " + str(form.name) + "!", "success")
                return redirect(url_for(gbot.views.task.task_detail, group_codename=group_codename, task_id=task_id))
            except Exception as e:
                flash("Oh shoot, that edit didn't go right.  Here's the deal: " + str(e), "error")
                return redirect(url_for(gbot.views.task.task_edit, group_codename=group_codename, task_id=task_id))
    else:
    ...
    if this_task.is_approving(current_member):
        ...
        # However, if something DOES get submitted, check the form to see if they want it gone.  Then do what it says.
            ...
            if form.delete:
                ...
                try:
                    gbot.controllers.task.delete_task(this_task.task_id)
                    flash("Okay, you just deleted the Task called " +task_name +".", "success")
                    return redirect(url_for(gbot.views.task.task_list, group_codename=group_codename))
                except Exception as e:
                    flash("Whoa, we couldn't delete the Task called " + this_task.name + "!  Check it out: " + str(e), "error")
                    return redirect(url_for(gbot.views.task.task_delete, group_codename=group_codename, task_id=this_task.task_id))
            else:
...
def task_deliver(group_codename, task_id):
    ...
    if this_task.is_delivering(current_member):
        ...
        # Unless it has, in which case call up the controller function, flash the appropriate message, and redirect.
            ...
            try:
                gbot.controllers.task.deliver_task(request, this_task.task_id, current_member.member_id)
                ...
                return redirect(url_for(gbot.views.task.task_deliver, group_codename=group_codename, task_id=task_id))
    else:
...
def task_approve(group_codename, task_id):
    ...
    if this_task.is_approving(current_member):
        ...
        # Unless they have, in which case call the controller function, flash some messages, and redirect.
            ...
            try:
                gbot.controllers.task.approve_task(request, task_id, current_member.member_id)
                ...
                return redirect(url_for(gbot.views.task.task_approve, group_codename=group_codename, task_id=task_id))
