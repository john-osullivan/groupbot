    ...
    if current_member is not None:
        ...
        # If somebody has, call the controller function, flash the appropriate message, and move the Member along.
            ...
            try:
                ...
                return redirect(url_for(views.event.event_detail, group_codename=group_codename, event_id=new_event))
            except Exception as e:
                flash("Oh VanDerShnootzle, the Event create didn't work!  Here's the dealio: " + str(e), "error")
                return redirect(url_for(views.event.event_create, group_codename=group_codename))
...
def event_detail(group_codename, event_id):
    ...
    (infonav, this_group, this_event) = views.get_context(group_codename=group_codename, event_id=event_id)
    ...
    # WHOA -- this person can't see that event!  Send 'em back to the Events page.
        ...
        return redirect(url_for(views.event.event_list, group_codename=group_codename))
...
def event_edit(group_codename, event_id):
    ...
    (infonav, this_group, this_event) = views.get_context(group_codename=group_codename, event_id=event_id)
    current_member = views.get_current_member(group_codename, current_user)
    ...
    if this_event.is_host(current_member):
        ...
        if form.validate_on_submit():
            ...
            try:
                ...
                return redirect(url_for(views.event.event_detail, group_codename=group_codename, event_id=event_id))

            # Unless shit went wrong, in which case say so.
            except Exception as e:
                flash("Yikes!  The edit failed for some reason, sorry.  Here's why: " + str(e), "error")
                return redirect(url_for(views.event.event_edit, group_codename=group_codename, event_id=event_id))
...
def event_rsvp(group_codename, event_id):
    ...
    (infonav, this_group, this_event) = views.get_context(group_codename=group_codename, event_id=event_id)
    current_member = views.get_current_member(this_group.codename, current_user)
    ...
    if this_event.is_invited(current_member):
        ...
        if form.validate_on_submit():
            ...
            # Unless, god forbid, something goes wrong.  Then say so and return to RSVP.
                ...
                return redirect(url_for(views.event.event_rsvp, group_codename=group_codename, event_id=event_id))
...
def event_attendance(group_codename, event_id):
    ...
    if this_event.is_host(current_member):
        ...
        form = forms.EventAttendanceForm()
        if form.validate_on_submit:
            ...
            try:
                ...
                return redirect(url_for(views.event.event_attendance, group_codename=group_codename, event_id=event_id))
            except Exception as e:
