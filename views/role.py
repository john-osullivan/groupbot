...
def role_create(group_codename):
    ...
    (current_group, current_member) = views.group_and_member_from_user_and_groupname(current_user.user_id, group_codename)
    infonav = views.build_infonav('group', current_group=current_group)
    ...
    if current_member != None:
        ...
        if form.validate_on_submit():
            ...
            try:
                new_role = controllers.role.create_role(current_group.group_id, request)
...
def role_list(group_codename):
    ...
    (this_group, infonav) = views.get_group_and_nav(group_codename)
...
def role_edit(group_codename, role_id):
    ...
    (this_group, this_member) = views.group_and_member_from_user_and_groupname(current_user.user_id, group_codename)
    infonav = views.build_infonav('group', current_group=this_group)
    ...
    if this_member != None:
        ...
        if form.validate_on_submit():
            ...
            try:
                edited_role = controllers.role.edit_role(role_id, request)
    ...
    # If we got here, they weren't a Member of the Group whose Role they were trying to edit!  PUNT 'EM.
        ...
        return redirect(url_for(views.group.group_list, infonav=infonav))
...
def role_delete(group_codename, role_id):
    ...
    if this_member != None:
        ...
        if form.validate_on_submit():
            ...
            try:
                deleted = controllers.role.delete_role(role_id, request)
                if deleted:
                    flash("You just deleted the Role named " + role_name + ".")
                    return redirect(url_for(views.role.role_list, group_codename=group_codename))
                else:
                    flash(role_name + " wasn't deleted -- phew.")
                    return redirect(url_for(views.role.role_detail, group_codename=group_codename, role_id=role_id))
