...
from app import app
import views
from models import Member, Group, Role, User
import forms
import controllers
...
def member_detail(group_codename, member_codename):
    ...
    (this_group, this_member) = views.get_group_and_member(group_codename, member_codename)
    infonav = views.build_infonav('group', current_group=this_group)
    ...
    if this_member.user_id == current_user.user_id:
        ...
        if form.validate_on_submit():
            ...
            try:
                ...
                return redirect(url_for(views.member.member_detail(group_codename, new_codename)))

            # Let them know what went wrong if it wasn't
            except Exception as e:
                flash("Dang, that edit didn't work...  Check it out: " + str(e))
                return redirect(url_for(views.member.member_edit(group_codename, member_codename)))
        ...
        if form.validate_on_submit():
            ...
            if form.signature_field.signature == current_user.codename:
                try:
                    ...
                    return redirect(url_for(views.member.member_delete(group_codename, member_codename)))
